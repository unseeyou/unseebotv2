...
class NumberGameButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.label = '\u200b'
        self.disabled = False
        view.log_action(old_total=old_total, current_num=current_num, new_total=total, new_num=self.num)
        content = f"**SUM:** {total}"

        if winner:
            self.gameover = True
        if self.gameover:
            view.log_action('[Game Won]')
...
class NumberGameGrid(discord.ui.View):
    ...
    def log_action(self, old_total, current_num, new_total, new_num):
        self.moves.append({"old_total": old_total,
...
class NumberGameCommand(commands.Cog):
    @commands.hybrid_command(help='unleash your big brain with this game')
